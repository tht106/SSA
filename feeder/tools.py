...
transform_order = {
  ...
  'azure': [0, 1, 2, 3, 11, 12, 13, 14, 15, 16, 17, 4, 5, 6, 7, 8, 9, 10, 22, 23, 24, 25,  18, 19, 20, 21, 26, 27, 28, 29, 30, 31, ]
}
...
class GaussianBlurConv(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = torch.tensor(x, dtype=torch.double)# torch.from_numpy(x) cause RuntimeError: Input type (torch.FloatTensor)
                                               # and weight type (torch.DoubleTensor) should be the same or input should
