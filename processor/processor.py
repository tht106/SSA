...
class Processor(IO):
    ...
    def init_environment(self):
        ...
        self.eval_info = dict()
        if self.arg.weights and 'pretext' in self.arg.config:
            restored_epoch = int(self.arg.weights.split('_')[1][-4:])
            self.meta_info = dict(epoch=restored_epoch, iter=0)
            self.arg.start_epoch = restored_epoch
        else:
    ...
    def load_data(self):
        ...
        if self.arg.train_feeder_args:
            ...
            self.data_loader['train'] = torch.utils.data.DataLoader(
                ...
                num_workers=0, #self.arg.num_worker * ngpu(self.arg.device),
                drop_last=True,
    ...
    def start(self):
        ...
        elif self.arg.phase == 'tsne':
            if self.arg.weights is None:
                raise ValueError('Please appoint --weights.')
            self.io.print_log('Model:   {}.'.format(self.arg.model))
            self.io.print_log('Weights: {}.'.format(self.arg.weights))
            self.io.print_log('Drawing TSNE... ')
