...
class AimCLR(nn.Module):
    def __init__(self, base_encoder=None, pretrain=True, feature_dim=128, queue_size=32768, STGCNplus=False, MSGCN=False,
        ...
        if not self.pretrain:
            ...
            if STGCNplus:
                self.encoder_q = base_encoder(num_classes=num_class, in_channels=in_channels, **kwargs)
            else:
                ...
                self.T = Temperature
                if STGCNplus:
                    self.encoder_q = base_encoder(num_classes=feature_dim, in_channels=in_channels, **kwargs)
                    self.encoder_k = base_encoder(num_classes=feature_dim, in_channels=in_channels, **kwargs)
                if MSGCN:
                    self.encoder_q = base_encoder(num_classes=feature_dim, dim=in_channels, graph_args=graph_args, **kwargs)
                    self.encoder_k = base_encoder(num_classes=feature_dim, dim=in_channels, graph_args=graph_args, **kwargs)
                else:
                    self.encoder_q = base_encoder(in_channels=in_channels, hidden_channels=hidden_channels,
    ...
    def _dequeue_and_enqueue(self, keys):
        ...
        gpu_index = 0 if self.queue.shape[0] == keys.shape[0] else keys.device.index
    ...
    def forward(self, im_q_extreme, im_q, im_k=None, nnm=False, topk=1):
        ...
        labels = torch.zeros(logits.shape[0], dtype=torch.long).to(logits.device)#cuda()
    ...
    def nearest_neighbors_mining(self, im_q, im_k, im_q_extreme, topk=1):
        ...
        pos_mask = torch.cat([torch.ones(topk_onehot.size(0), 1).to(logits.device), topk_onehot], dim=1)  #.cuda(), topk_onehot], dim=1)
