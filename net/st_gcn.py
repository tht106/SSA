...
class Model(nn.Module):
    ...
    def forward(self, x, drop=False, output_feat=False):
        ...
        if drop:
            ...
            # global pooling
            # x = torch.Size([256, 256, 13, 25])
            x = F.avg_pool2d(x, x.size()[2:])  # torch.Size([256, 256, 1, 1]
            x = x.view(N, M, -1).mean(dim=1)  # torch.Size([128, 256]
            if output_feat:
                return x
            # prediction
            x = self.fc(x)  # torch.Size([128, action_categories]
            x = x.view(x.size(0), -1)
